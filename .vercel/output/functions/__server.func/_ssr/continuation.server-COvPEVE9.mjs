//#region node_modules/.nitro/vite/services/ssr/assets/continuation.server-COvPEVE9.js
async function buildContinuationContext(supabase, programId, nHistory, cooldown = 3) {
	const { data: weeks } = await supabase.from("program_weeks").select("id, numero_semana").eq("program_id", programId).order("numero_semana", { ascending: true });
	const weekIds = (weeks ?? []).map((w) => w.id);
	const lastWeekNumber = weeks?.length ? weeks[weeks.length - 1].numero_semana : 0;
	if (weekIds.length === 0 || nHistory === 0) return {
		version: 1,
		programId,
	...
	const { data: sessoes } = await supabase.from("sessions").select("id, titulo, numero_dia, program_week_id, program_weeks(numero_semana)").in("program_week_id", weekIds).order("created_at", { ascending: false }).limit(nHistory);
	...
	const { data: blocos } = await supabase.from("session_blocks").select("id, session_id, formato, titulo, ordem, config").in("session_id", sessaoIds).order("ordem", { ascending: true });
	const blocoIds = (blocos ?? []).map((b) => b.id);
	const { data: exercicios } = await supabase.from("session_block_exercises").select(`
      session_block_id, 
      nome_livre, 
      exercise_id,
      exercises(id, nome_pt, grupo_muscular)
    `).in("session_block_id", blocoIds).order("ordem", { ascending: true });
	...
	const lastSessionStructure = (blocos ?? []).filter((b) => b.session_id === lastSessao.id).sort((a, b) => (a.ordem || 0) - (b.ordem || 0)).map((b) => {
	...
	const usage = Array.from(usageMap.values()).map((u) => ({
		...
		name: u.name,
		count: u.count,
		lastSeenSessionIdx: u.lastIdx
	})).sort((a, b) => b.count - a.count);
	const softAvoidIds = usage.filter((u) => sessoes.length - 1 - u.lastSeenSessionIdx < cooldown).map((u) => u.exerciseId);
...
//#endregion
export { buildContinuationContext };
