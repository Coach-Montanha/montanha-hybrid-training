...
export async function checkProjectAccess(
  ...
  if (typeof window !== 'undefined') {
    ...
    // Support trial or impersonation session active locally
    const trialUser = localStorage.getItem('sistema_hibrido_trial_user') || localStorage.getItem('edufinance.impersonate');
    if (trialUser) {
      return {
        hasAccess: true,
        status: 'AVALIAÇÃO',
        expiresAt: null,
        message: 'Acesso liberado em período de avaliação / suporte.'
      };
    }
  }
...
export const APP_NAMES_MAP: Record<string, { name: string; url: string }> = {
  ...
  'all': { name: 'Ecossistema Montanha (5 Apps)', url: 'http://localhost:5176' }
};
